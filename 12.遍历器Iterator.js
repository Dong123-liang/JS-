  ...
  return {
    next() {
      return nextIndex < array.length
        ? { value: array[nextIndex++], done: false } //done:false表示继续执行，为true表示遍历完
...
*/
let arr=['guiji',1,22,21,'hello']
let keys = arr.values();
let key =arr.keys();
let {value,done} = keys.next(); 
console.log(value,done);//guiji flase 读取每一步的值
console.log(key.next());//{ value: 0, done: false } 此处的value表示的索引
console.log(key.next());//{ value: 1, done: false }
console.log(key.next());//{ value: 2, done: false }
//可以用 for of 进行迭代
for(const content of arr.values()){
  console.log(content);//迭代所有的值
}
for(const key of arr.keys()){
  console.log(key);//迭代出索引号
}